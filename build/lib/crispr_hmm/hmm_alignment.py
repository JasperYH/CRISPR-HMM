...
class hmm_model(object):
    ...
    def calculate_alignment_probability(self, aligned_ref, aligned_seq, use_log=False):
        """
    ...
    def align_seqs_with_viterbi(self, seqs, ncores):
        pool = mp.Pool(ncores)
        with pool:
            results = pool.map(self.viterbi, seqs)
        return results
    ...
    def Baum_Welch_estimator(self,seqs,method="Constrained_MLE",\
        ...
        for i in range(MAX_iter+1):
            ...
            self._update_parameters(method, param_hat)
    ...
    def _update_parameters_adaptive_step_size(self, param, param_hat, lb):
    ...
    def _update_parameters(self, method, param_hat):
        if method == "Unconstrained_MLE":
            for param in param_hat:
            ...
            if "delta" in param_hat and "tau" in param_hat:
                ...
                for i in range(self.n+1):
                    ...
                    tmp = self._update_parameters_adaptive_step_size(param, p_hat, lb)
